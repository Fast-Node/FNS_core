...
public:
    ...
    bool WriteZFNSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZFNSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZFNSSeed_deprecated(uint256& seed);
    bool EraseZFNSSeed();
    bool EraseZFNSSeed_deprecated();

    bool WriteZFNSCount(const uint32_t& nCount);
    bool ReadZFNSCount(uint32_t& nCount);
