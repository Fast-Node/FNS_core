...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FNS);
    ...
    if (!settings.contains("nAnonymizeFastnodeAmount"))
        settings.setValue("nAnonymizeFastnodeAmount", 1000);

    nAnonymizeFastnodeAmount = settings.value("nAnonymizeFastnodeAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeFastnodeAmount"))
        SoftSetArg("-anonymizefastnodeamount", settings.value("nAnonymizeFastnodeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in fastnode.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFastnodeAmount:
            return QVariant(nAnonymizeFastnodeAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFastnodeAmount:
            nAnonymizeFastnodeAmount = value.toInt();
            settings.setValue("nAnonymizeFastnodeAmount", nAnonymizeFastnodeAmount);
            emit anonymizeFastnodeAmountChanged(nAnonymizeFastnodeAmount);
            break;
        case CoinControlFeatures:
