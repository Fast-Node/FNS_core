...
// The zFNS block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zFNS is a hash of the serial
...
{
    //Create an output returning the zFNS that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZFNSOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zFNS output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zFNS", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZFNSOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zFNS output", __func__);
...
{
    CzFNSTracker* zfnsTracker = pwallet->zfnsTracker.get();
    CMintMeta meta;
    if (!zfnsTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zfnsTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a FNS stake is the outpoint
