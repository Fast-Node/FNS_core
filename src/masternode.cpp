    ...
    if (!unitTest) {
        ...
        if(GetTransaction(vin.prevout.hash, wtx2, hashBlock2, true)) {
            ...
            if (iter != mapBlockIndex.end()) {
              ...
              if (txnheight <= GetSporkValue(SPORK_18_COLLATERAL_1000)) {
                  collateral = 1000 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_19_COLLATERAL_1500)) {
                  collateral = 1500 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_20_COLLATERAL_2000)) {
                  collateral = 2000 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_21_COLLATERAL_2500)) {
                  collateral = 2500 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_22_COLLATERAL_3000)) {
                  collateral = 3000 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_23_COLLATERAL_3500)) {
                  collateral = 3500 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_24_COLLATERAL_4000)) {
                  collateral = 4000 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_25_COLLATERAL_4500)) {
                  collateral = 4500 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_26_COLLATERAL_5500)) {
                  collateral = 5500 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_27_COLLATERAL_6500)) {
                  collateral = 6500 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_28_COLLATERAL_7500)) {
                  collateral = 7500 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_29_COLLATERAL_8500)) {
                  collateral = 8500 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_30_COLLATERAL_9500)) {
                  collateral = 9500 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_31_COLLATERAL_10000)) {
                  collateral = 10000 * COIN;
              } else if (txnheight <= GetSporkValue(SPORK_32_COLLATERAL_20000)) {
                  collateral = 20000 * COIN;
              }
            }
        }
        //CTxOut vout = CTxOut(collateral - 0.01, masternodeSigner.collateralPubKey);
        //CTxOut vout = CTxOut(((GetMstrNodCollateral() - 0.01)) * COIN, obfuScationPool.collateralPubKey);
        CTxOut vout = CTxOut(collateral - 0.01, obfuScationPool.collateralPubKey);
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 47352) return false;
    } else if (addr.GetPort() == 47352)
        return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 FNS tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 FASTNODE tx -> 1 confirmation
